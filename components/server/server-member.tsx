...
export const ServerMember:FC<ServerMemberProps> = ({
    ...
    const onClick = () => {
        router.push(`/servers/${params?.server}/conversations/${member.id}`)
    }
 ...
 return (
    <button
        onClick={onClick}
        className={cn(
            "group px-2 py-2 rounded-md flex items-center gap-x-2 w-full hover:bg-zinc-700/10 dark:hover:bg-zinc-700/50 transition mb-1 ",params?.memberId === member.id && "bg-zinc-700/50 dark:bg-zinc-700"
        )}
    >
        <UserAvatar
